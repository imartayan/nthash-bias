...
cargo r -r -- -r 1 7 -t 4 | python plot-bias.py -f png

# passing -i <input.fa> will use a specific sequence instead
cargo r -r -- -r 1 7 -t 4 -i <input.fa> | python plot-bias.py -f png

# output multiple plot formats
...
A simple benchmark measuring the throughput of different *k*-mer hashing methods is available with:
```sh
cargo r -r --bin throughput
```
